...
void setup(void)
{
	...
	if (force_menu || EEPROM.read(EEPROM_VERSION) < 23 || EEPROM.read(EEPROM_VERSION) == 255 || (EEPROM.read(EEPROM_DISPLAY) != BGR_DISP && EEPROM.read(EEPROM_DISPLAY) != RGB_DISP))
	{
		...
		tft.print("Welcome!");
		delay(4000);
		while (!digitalRead(SW_T3) || !digitalRead(SW_T1))
			delay(100);
	}
	else
	{
	...
	if (EEPROM.read(EEPROM_ADCTTG) == 255)
	{ // Override unset values from older versions
		EEPROM.put(EEPROM_ADCTTG, adc_gain);
		EEPROM.put(EEPROM_ADCOFF, adc_offset);
	}
...
#endif
	if (adc >= 900)
	{ // Illegal value, tip not plugged in - would be around 560deg
...
void display(void)
{
	...
	tft.drawCircle(20, 63, 8, off ? RED : yell ? YELLOW
											   : GREEN);
	tft.drawCircle(20, 63, 7, off ? RED : yell ? YELLOW
											   : GREEN);
	tft.fillRect(19, 55, 3, 3, BLACK);
	tft.drawFastVLine(20, 53, 10, off ? RED : yell ? YELLOW
												   : GREEN);
	if (error != NO_ERROR)
	{
